...
CREATE OR REPLACE FUNCTION create_history_table (tablename text) RETURNS void AS $$
	...
	-- note: this will have to be further modified if we use 'geometry'
	-- datatypes in postgis.
	FOR field IN SELECT ic.table_name, ic.column_name, ic.data_type, ic.is_nullable, ic.udt_name, gc.srid, gc.type FROM information_schema.columns AS ic LEFT JOIN geography_columns AS gc ON ic.table_name = gc.f_table_name AND ic.column_name = gc.f_geography_column WHERE table_name = tablename LOOP
	    IF field.srid IS NULL THEN 
		  ...
		  create_statement := create_statement || ' NOT NULL';
	       END IF;	  	
	    ELSE
		create_statement := create_statement || ', ' || field.column_name || ' geography(' || field.type || ', ' || field.srid || ')';
	       IF field.is_nullable = 'NO' THEN
		  create_statement := create_statement || ' NOT NULL';
	       END IF;	  	
	    END IF;
