...
public class MqttService extends Service implements MqttTraceHandler {
  ...
  public String getClient(String serverURI, String clientId, MqttClientPersistence persistence) {
    ...
    if (!clients.containsKey(clientHandle)) {
      Log.d(TAG, "Creating new client for clientHandle " + clientHandle);
  ...
  public void connect(String clientHandle, MqttConnectOptions connectOptions,
    ...
    Log.d(TAG, "Connecting to clientHandle " + clientHandle);
  ...
  public void disconnect(String clientHandle, String invocationContext,
    ...
    Log.d(TAG,"Disconnecting client " + clientHandle + " (1)");
    MqttServiceClient client = clientFromHandle(clientHandle);
  ...
  public void disconnect(String clientHandle, long quiesceTimeout,
      String invocationContext, String activityToken) {
    Log.d(TAG,"Disconnecting client " + clientHandle + " (2)");
    MqttServiceClient client = clientFromHandle(clientHandle);
  ...
  /**
   * disconnect forcibly from the server
   * 
   * @param clientHandle
   *            identifies the MqttServiceClient to use
   * @param invocationContext
   ...
   *            arbitrary identifier to be passed back to the Activity
   */
  public void disconnectForcibly(String clientHandle) {
    Log.d(TAG,"Forcibly disconnecting client " + clientHandle + " (1)");
    MqttServiceClient client = clientFromHandle(clientHandle);
   ...
   *            arbitrary identifier to be passed back to the Activity
   */
  public void disconnectForcibly(String clientHandle, long disconnectTimeout) {
    Log.d(TAG,"Forcibly disconnecting client " + clientHandle + " (2)");
    MqttServiceClient client = clientFromHandle(clientHandle);
   ...
   */
  public void disconnectForcibly(String clientHandle,
      long disconnectTimeout, long quiesceTimeout) {
    Log.d(TAG,"Forcibly disconnecting client " + clientHandle + " (2)");
    MqttServiceClient client = clientFromHandle(clientHandle);
    client.disconnectForcibly(disconnectTimeout, quiesceTimeout);
    clients.remove(clientHandle);
